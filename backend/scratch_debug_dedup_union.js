...
async function check() {
  try {
    ...
    // Query with UNION instead of UNION ALL for detail lines
    ...
    console.log(`Fetched ${rows.length} unique rows.`);

    const tests = [
      { dStatus: [0, 1, 2, 4], taf: false, daf: false, isf: false, name: "No filters (UNION)" },
      { dStatus: [0, 1, 2, 4], taf: true, daf: false, isf: true, name: "InvoiceStatus=5 and InvoiceTotalAmount<>0" },
