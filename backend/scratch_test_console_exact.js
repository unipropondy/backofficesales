...
async function test() {
    ...
    console.log("--- Category Sales Query ---");
    ...
    `;
    const metaRes = await pool.request().input('start', sql.Date, start).input('end', sql.Date, end).query(metaQuery);
    console.table(metaRes.recordset);
}
