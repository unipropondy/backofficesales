...
async function checkDateMismatch() {
    try {
        ...
        // Query 1: Using >= and <= with string literals
        const q1 = `
            SELECT 
        ...
        `;
        const res1 = await pool.request().query(q1);
        console.log("Q1 (>= AND <=) =>", res1.recordset[0]);

        // Query 2: Using CAST as DATE
        const q2 = `
            SELECT 
                SUM(ROD.Quantity) AS TotalQty,
                SUM(ROD.TotalDetailLineAmount) AS TotalAmt
        ...
        console.log("Q2 (CAST BETWEEN) =>", res2.recordset[0]);
