...
const vwOrderDetailUnion = `
    ...
    FROM (
        SELECT OrderId, DishId, Quantity, BaseAmount, ManualDiscountAmount, TotalDetailLineAmount FROM dbo.RestaurantOrderDetail
        UNION ALL
        SELECT OrderId, DishId, Quantity, BaseAmount, ManualDiscountAmount, TotalDetailLineAmount FROM dbo.RestaurantOrderDetailCur
    ) od
    INNER JOIN (
...
async function testRawTables() {
    try {
        ...
        console.log("Summary Query Result with RAW tables:", res.recordset[0]);
